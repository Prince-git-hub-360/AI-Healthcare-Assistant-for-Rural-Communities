...
export const CaregiverNavbar = ({ currentView, setCurrentView, onOpenChat }) => {
  const { user, logout, currentLang, updateLanguage } = useAuth();
  const [drawerOpen, setDrawerOpen] = useState(false);

  const roleEmoji = '👨‍👩‍👧 Caregiver';

  const navItems = [
    { id: 'dashboard', label: 'Caregiver Hub', icon: HospitalIcon },
    { id: 'translate', label: 'Translate Rx', icon: TranslateIcon },
    { id: 'medical_vault', label: 'Health Vault', icon: DocumentIcon },
    { id: 'reminders', label: 'Dose Reminders', icon: ClockIcon },
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#fdfbf7]/95 backdrop-blur-md border-b border-stone-200 shadow-sm">
      <div className="max-w-7xl mx-auto flex items-center justify-between py-3 px-4 md:px-8">
        ...
        >
          <div className="w-10 h-10 bg-[#0F766E] text-white rounded-xl flex items-center justify-center relative shadow-sm">
          ...
          <div>
            ...
            <span className="text-[9px] font-extrabold text-[#0F766E] tracking-wider uppercase block mt-1">
              CAREGIVER PORTAL
            </span>
          </div>
        </div>
            ...
            <div>
              <div className="flex justify-between items-center pb-4 border-b border-stone-200 mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-xs font-extrabold text-stone-900">{user?.first_name || user?.username || 'Caregiver'}</div>
