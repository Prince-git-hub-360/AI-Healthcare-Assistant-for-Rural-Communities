...
export const WorkerNavbar = ({ currentView, setCurrentView, onOpenChat }) => {
  const { user, logout, currentLang, updateLanguage } = useAuth();
  const [drawerOpen, setDrawerOpen] = useState(false);

  const roleEmoji = user?.role === 'doctor' ? '👨‍⚕️ Doctor' : '👩‍⚕️ Doctor / ASHA';

  const navItems = [
    { id: 'dashboard', label: 'Field Portal', icon: HospitalIcon },
    { id: 'translate', label: 'Translate Rx', icon: TranslateIcon },
    { id: 'medical_vault', label: 'Health Vault', icon: DocumentIcon },
    { id: 'reminders', label: 'Follow-ups', icon: ClockIcon },
              ...
              <div className="flex justify-between items-center pb-4 border-b border-stone-200 mb-6">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-teal-700 text-white rounded-lg flex items-center justify-center font-bold text-xs">
                    {user?.first_name ? user.first_name.charAt(0).toUpperCase() : 'W'}
                  </div>
                  <div>
                    <div className="text-xs font-extrabold text-stone-900">{user?.first_name || user?.username || 'Worker'}</div>
