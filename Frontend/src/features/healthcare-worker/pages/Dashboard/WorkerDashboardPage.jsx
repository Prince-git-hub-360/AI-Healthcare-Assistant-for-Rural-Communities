    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-stone-200 p-5 rounded-3xl shadow-xs">
          <span className="text-[10px] font-extrabold text-stone-400 uppercase tracking-wider block mb-1">Catchment Population</span>
          <div className="text-3xl font-extrabold text-teal-800">{fieldPatients.length || 42}</div>
          <div className="text-xs font-semibold text-stone-600 mt-1">Registered Village Patients</div>
        </div>

        <div className="bg-red-50 border border-red-200 p-5 rounded-3xl shadow-xs">
          <span className="text-[10px] font-extrabold text-red-700 uppercase tracking-wider block mb-1">High-Risk Alerts</span>
          <div className="text-3xl font-extrabold text-red-600">3</div>
          <div className="text-xs font-bold text-red-900 mt-1">Require Home Visit Today 🏠</div>
        </div>

        <div className="bg-amber-50 border border-amber-200 p-5 rounded-3xl shadow-xs">
          <span className="text-[10px] font-extrabold text-amber-800 uppercase tracking-wider block mb-1">Pending Follow-ups</span>
          <div className="text-3xl font-extrabold text-amber-600">6</div>
          <div className="text-xs font-semibold text-amber-900 mt-1">Prescription Renewals Due</div>
        </div>

        <div className="bg-emerald-50 border border-emerald-200 p-5 rounded-3xl shadow-xs">
          <span className="text-[10px] font-extrabold text-emerald-800 uppercase tracking-wider block mb-1">2G IVR Adherence</span>
          <div className="text-3xl font-extrabold text-emerald-700">94.2%</div>
          <div className="text-xs font-semibold text-emerald-900 mt-1">Automated Call Success</div>
        </div>
      </div>
      ...
      <div className="bg-white border border-stone-200 rounded-3xl p-6 md:p-8 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-2 border-b border-stone-100">
          <div>
            ...
            <p className="text-xs text-stone-500">Sorted by Risk Status • Filtered for ASHA Worker Catchment Area</p>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-stone-600">Filter Village:</span>
            <select className="bg-stone-50 border border-stone-300 text-stone-900 text-xs rounded-xl px-3 py-1.5 outline-none font-semibold">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-stone-200 text-stone-500 font-extrabold uppercase">
            ...
            <tbody className="divide-y divide-stone-100">
              <tr className="bg-red-50/50 hover:bg-red-50 transition-colors">
                <td className="py-3.5 px-3">
                  <div className="font-extrabold text-stone-900 text-sm">Sunita Devi</div>
                  <div className="text-[11px] text-stone-500">+91 98765 43210 (2G Phone)</div>
                </td>
                <td className="py-3.5 px-3 font-semibold text-stone-700">Mandya Sector 2</td>
                <td className="py-3.5 px-3 font-bold uppercase text-teal-800">Hindi (hi)</td>
                <td className="py-3.5 px-3">
                ...
                <td className="py-3.5 px-3 space-x-2">
                  <button
                    ...
                    className="bg-teal-700 hover:bg-teal-800 text-white font-bold px-3 py-1.5 rounded-xl text-[11px] cursor-pointer"
                  >
                    📸 Scan Rx Note
                  </button>
                  <button
              ...
              {fieldPatients.map((p, idx) => (
                <tr key={idx} className="hover:bg-stone-50 transition-colors">
                  <td className="py-3.5 px-3">
                    ...
                    <div className="text-[11px] text-stone-500">{p.phone_number || '+91 9876500111'}</div>
                  </td>
                  <td className="py-3.5 px-3 text-stone-600 font-medium">{p.village_or_town || 'Mandya'}</td>
                  <td className="py-3.5 px-3 text-stone-600 uppercase font-semibold">{p.preferred_language || 'kn'}</td>
                  <td className="py-3.5 px-3">
                  ...
                  <td className="py-3.5 px-3 space-x-2">
                    <button
                      ...
                      className="bg-teal-50 hover:bg-teal-100 text-teal-800 font-bold px-3 py-1.5 rounded-xl border border-teal-200 text-[11px] cursor-pointer"
                    >
                      Upload Rx
                    </button>
                    <button
                      onClick={() => showToast?.(`Sending 2G IVR Call Alert to ${p.first_name}...`, 'info')}
                      className="bg-slate-900 hover:bg-slate-800 text-teal-300 font-bold px-3 py-1.5 rounded-xl text-[11px] cursor-pointer"
                    >
                      📞 2G IVR Call
                    </button>
                  </td>
