...
import { ShieldIcon, UserIcon, HospitalIcon, BrainIcon } from '../../../../shared/icons/Icons';

export const AboutPage = ({ setCurrentView, openAuthModal, onNavigate }) => {
  const handleRegister = () => {
    if (onNavigate) onNavigate('/register');
    else if (openAuthModal) openAuthModal('register');
  };

  const handleHome = () => {
    if (onNavigate) onNavigate('/');
    else if (setCurrentView) setCurrentView('landing');
  };
  return (
    <div className="landing-section" style={{ paddingTop: '60px', minHeight: '80vh' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <button className="btn-cta-coral cursor-pointer" onClick={handleRegister}>
          Get Started Free →
        </button>
        <button className="btn-outline-teal cursor-pointer" onClick={handleHome} style={{ marginLeft: '16px' }}>
          Return to Homepage
        </button>
      </div>
    </div>
